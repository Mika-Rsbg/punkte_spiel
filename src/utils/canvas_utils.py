...
def highlight_field(canvas, col=None, row=None, color="lightgray",
                    fields=config.GameGrid.fields, field_coords=None):
    """Markiert ein Feld mit einer Farbe, überschreibt persistente Highlights
        temporär.

    Args:
        canvas (tk.Canvas): Spiel-Canvas.
        col (int, optional): Spalten-Koordinate des Feldes.
        row (int, optional): Zeilen-Koordinate des Feldes.
        color (str, optional): Farbe. Defaults to "lightgray".
        fields (dict): Dictionary mit Feldinformationen.
        field_coords (tuple): Koordinaten des Feldes.
    """
    # Feld-Koordinaten berechnen
    if field_coords is None:
        if col is None or row is None:
            raise ValueError(
                "Entweder 'field_coords' oder 'col' und 'row' müssen "
                "angegeben werden.")
...
def clear_field(canvas, col=None, row=None, fields=config.GameGrid.fields,
                field_coords=None):
...
def clear_all_fields(canvas, rows=None, cols=None,
    ...
    Args:
        ...
        fields (dict, optional): Dictionary in dem die Felder eines Grids
            gespeichert sind. Defaults to `config.Grid.fields`
    """
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            # Wenn eine bestehende Farbe vorhanden ist, stelle diese wieder her
            persistent_color = fields[(col, row)].get("persistent_color")
            if persistent_color:
                canvas.itemconfig(rect_id, fill=persistent_color)
            else:
                canvas.itemconfig(rect_id,
                                  fill=config.GameCanvas.CANVAS_BACKGROUND)


def persistent_highlight_field(canvas, col=None, row=None, color=None,
                               fields=config.GameGrid.fields,
                               field_coords=None, transparently=True,
                               alpha=0.5):
    ...
    Args:
        ...
        fields (dict): Dictionary mit Feldinformationen.
        field_coords (tuple): Koordinaten des Feldes.
        transparently (bool): Soll das Feldt halbtransparent makiert werden.
            Defaults to `True`.
    """
    # Feld-Koordinaten berechnen
    if field_coords is None:
...
def clear_persistent_highlight(canvas, field_to_remove, fields_to_remove=None,
                               fields=config.GameGrid.fields):
    """Entfernt spezifische Felder aus der Liste der persistent
        hervorgehobenen Felder, falls sie enthalten sind.
    Args:
        canvas (parameter): Spiel Canvas.
        field_to_remove (tuple): Koordintaten des Feldes, das entfernt
            werden soll.
        fields_to_remove (list of tuple): Liste der Koordinaten der Felder,
            die entfernt werden sollen, z. B. [(col1, row1), (col2, row2)].
        fields (dict, optional): Dictionary mit den Feldern des Grids.
            Defaults to `config.GameGrid.fields`.
    """
    if fields_to_remove is None:
        fields_to_remove = [field_to_remove]
    for field_coords in fields_to_remove:
        if field_coords in fields:
            rect_id = fields[field_coords]["rect_id"] if isinstance(
                fields[field_coords], dict) else fields[field_coords]
            # Wenn das Feld in persistent_highlights
            # enthalten ist, zurücksetzen und entfernen
            if not fields[field_coords]["persistent_color"] is None:
                canvas.itemconfig(
                    rect_id, fill=config.GameCanvas.CANVAS_BACKGROUND
                )
                config.GameGrid.fields[field_coords]["persistent_color"] = None


def clear_all_persistent_highlights(canvas, rows=None, cols=None,
                                    fields=config.GameGrid.fields):
...
def transparently_highlight_field(canvas, col=None, row=None,
                                  color=config.Design.Colors.LIGHT, alpha=0.5,
                                  fields=config.GameGrid.fields,
                                  field_coords=None):
    """Markiert ein Feld an den angegebenen Grid-Koordinaten mit einer
        halbtransparenten Färbung.

    Args:
        canvas (tk.Canvas): Spiel-Canvas.
        col (int, optional): Spalten-Koordinate des Feldes.
        row (int, optional): Zeilen-Koordinate des Feldes.
        color (str, optional): Farbe im HEX-Format.
            Defaults to `config.Design.Colors.LIGHT` (LightGray).
        alpha (float, optional): Transparenzwert zwischen 0
            (komplett transparent) und 1 (voll deckend). Defaults to 0.5.
        fields (dict, optional): Dictionary mit Feldern des Grids.
            Defaults to config.GameGrid.fields.
        field_coords (tuple, optional): Koordinaten des Feldes als
            (Spalte, Zeile).
    """
    if field_coords is None:
        if col is None or row is None:
...
def blend_colors(bg_color, fg_color, alpha):
    ...
    Args:
        ...
        alpha (float): Transparenzwert (0 = vollständig durchsichtig,
...
class draw:
    ...
    @staticmethod
    def draw_ressourcenproduktion(canvas, x=None, y=None, col=None, row=None,
                                  field_coords=None, color=None,
                                  grid_size=config.GameGrid.GRID_SIZE):
        """Zeichnet eine Ressourcenproduktion als Quadrat auf dem
            Spielfeld-Canvas.
        Args:
            canvas (tk.Canvas): Das Tkinter Canvas-Objekt,
                auf dem das Dorf gezeichnet wird.
            x (int, optional): Die x-Koordinate des Mittelpunktes des Quadrat.
            y (int, optional): Die y-Koordinate des Mittelpunktes des Quadrat.
            col (int, optional): Die Spaltennummer des Feldes,
                auf dem die Ressourcenproduktion liegt.
            row (int, optional): Die Zeilennummer des Feldes,
                auf dem die Ressourcenproduktion liegt.
            field_coords (tuple): Die Koordinaten des Feldes als
                (Spalte, Zeile).
            color (str): Die Farbe des Quadrat, z.B. "red", "green", etc.
        Returns:
            int: Die ID des gezeichneten Objekts im Canvas, die verwendet
                werden kann, um das Objekt später zu manipulieren."""
        if color is None:
        ...
        return canvas.create_rectangle(
            x - 10, y - 10, x + 10, y + 10, fill=color, tags=tags
        )

    @staticmethod
    def draw_stadt(canvas, x=None, y=None, col=None, row=None,
                   field_coords=None, color=None,
                   radius=config.Building.Stadt.RADIUS_STADT,
                   grid_size=config.GameGrid.GRID_SIZE):
        ...
        Args:
            ...
            color (str): Die Farbe des Kreises, z.B. "red", "green", etc.
            radius (int, optional): Der Radius des Kreises.
                Defaults to `config.Stadt.RADIUS_STADT`.
        Returns:
            int: Die ID des gezeichneten Objekts im Canvas, die verwendet
                werden kann, um das Objekt später zu manipulieren."""
        if color is None:
    ...
    @staticmethod
    def draw_road(canvas, start_coords, end_coords, thickness=8,
                  outline_color=None,
                  grid_size=config.GameGrid.GRID_SIZE, color=None):
        """Zeichnet eine Straße als dicken Strich von der Mitte eines Feldes
            zur Mitte eines benachbarten Feldes.

        Args:
            canvas (tk.Canvas): Das Tkinter Canvas-Objekt,
                auf dem die Straße gezeichnet wird.
            ...
            grid_size (int): Die Größe eines Feldes im Raster.
        ...
        if outline_color is None:
            outline_color = game_state_utils.get_player_color(
                config.Game.current_turn
            )
        ...
        if outline_color:
            outline_id = canvas.create_line(
                ...
                fill=outline_color, width=outline_thickness,
                tags="road_outline"
            )
    ...
    # def draw_road_segment(canvas, col, row, direction, thickness=8,
    #                       outline_color=None, color=None):
        """Zeichnet einen Straßenabschnitt innerhalb eines Feldes basierend
            auf der Richtung.
