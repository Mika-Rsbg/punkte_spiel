...
class Stadt:
    ...
    def occupy_area(col, row):
        occupation_radius = config.Building.Stadt.OCCUPATION_RADIUS
    ...
    @classmethod
    def build_stadt(cls):
        """Baut(Zeichnet) Stadt auf dem GameCanvas-Canvas."""
        ...
        config.GameGrid.fields[(col, row)]["building"] = config.Building.BuildingID.STADT
        ...
        economy_manager.pay_building(config.Building.BuildingID.STADT)
        cls.occupy_area(col, row)
        game_state_utils.get_player().number_of_stadte += 1
